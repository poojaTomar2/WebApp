...
Ext.extend(Ext.ux.Andrie.pPageSize, Ext.util.Observable, {
	...
	*/
	...
	setPageSize: function(value, forced) {
		...
		this.fireEvent("beforePageSizeChange", this, { value: this.combo.getValue() });
	...
	onRender: function() {
		...
		this.combo.on('change', this.setPageSize, this);
		this.updateStore();

		var toAdd = [], pagingToolbar = this.pagingToolbar;
		if (this.addBefore) {
			toAdd.push(this.addBefore);
		}
		if (this.beforeText) {
			toAdd.push(this.beforeText);
		}
		toAdd.push(this.combo);
		if (this.afterText) {
			toAdd.push(this.afterText);
		}
		if (this.addAfter) {
			toAdd.push(this.addAfter);
		}
		pagingToolbar.addCustomItems(toAdd);
	}
})
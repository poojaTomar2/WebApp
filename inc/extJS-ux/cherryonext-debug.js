  ...
  createFieldFromCfg: function(fieldCfg){
    ...
    switch(fieldCfg.type){
      ...
      case "int":
        ...
        break;
       case "date":
        field=new Ext.ux.netbox.date.DateField(fieldCfg.id,fieldCfg.label,fieldCfg.format,fieldCfg.convert);
        break;
      default:
        return(null);
    }
...
// $Id: DynamicFilterModelView.js 204 2009-06-25 08:56:50Z dandfra $
  ...
  createEditor: function(operatorId){
    ...
    if(splittedFormat.length > 1){
    	var dateTimeField = new Ext.ux.form.DateTime({
    			dateValid: true, // TLS #3810 - Manish
...
Ext.ux.netbox.date.DatePeriodOperator = function() {
	...
	this.periodStore = new Ext.data.SimpleStore({ fields: ['value', 'label'],
		data: [
		 ...
		 ["NEXT_HOUR", this.nextHourText],
		 ["CURRENT_DAY", this.currentDayText],
         ["LAST_DAY", this.dayText],
		 ["NEXT_DAY", this.nextdayText],
         ["LAST_24HOUR", this.last24HoursText],
		 ["NEXT_24HOUR", this.next24HoursText],
		 ["CURRENT_WEEK", this.currentWeekText],
         ["LAST_WEEK", this.weekText],
		 ["NEXT_WEEK", this.nextWeekText],
         ["LAST_7DAYS", this.last7DaysText],
		 ["NEXT_7DAYS", this.next7DaysText],
		 ["CURRENT_MONTH", this.currentMonthText],
         ["LAST_MONTH", this.monthText],
		 ["NEXT_MONTH", this.nextMonthText],
         ["LAST_30DAYS", this.last30DaysText],
		 ["NEXT_30DAYS", this.next30DaysText],
		 ["CURRENT_QUARTER", this.currentQuarterText],
         ["LAST_QUARTER", this.quarterText],
         ["LAST_90DAYS", this.last90DaysText],
		 ["CURRENT_YEAR", this.currentYearText],
    	 ["LAST_YEAR", this.yearText],
         ["LAST_365DAYS", this.last365DaysText]
      ]
	});
	var validateFn = function(value) {
		...
		if (value[0].value !== "LAST_QUARTER" && value[0].value !== "LAST_HOUR" && value[0].value !== "LAST_DAY"
		   ...
		   && value[0].value !== "LAST_24HOUR" && value[0].value !== "LAST_7DAYS" && value[0].value !== "LAST_30DAYS"
		   && value[0].value !== "LAST_90DAYS" && value[0].value !== "LAST_365DAYS" && value[0].value !== "CURRENT_DAY"
		   && value[0].value !== "CURRENT_WEEK" && value[0].value !== "CURRENT_MONTH" && value[0].value !== "CURRENT_QUARTER"
		   && value[0].value !== "CURRENT_YEAR" && value[0].value !== "NEXT_HOUR" && value[0].value !== "NEXT_DAY" && value[0].value !== "NEXT_24HOUR"
		   && value[0].value !== "NEXT_WEEK" && value[0].value !== "NEXT_7DAYS" && value[0].value !== "NEXT_MONTH" && value[0].value !== "NEXT_30DAYS") {
...
Ext.extend(Ext.ux.netbox.date.DatePeriodOperator, Ext.ux.netbox.core.Operator, {
	...
	last365DaysText		: "last 365 days",
	last30DaysText		: "last 30 days",
	last7DaysText		: "last 7 days",
	last24HoursText		: "last 24 hours",
	...
	next24HoursText		: "Next 24 hour", 
	nextWeekText		: "Next Week", 
	next7DaysText		: "Next 7 Days", 
	nextMonthText		: "Next Month",
	next30DaysText		: "Next 30 Days", 
...
Ext.ux.netbox.date.DateField = function (id, label, format,convert) {
	...
	op.addValidateFn(noEmptyAllowed);
	this.addOperator(op);
	op = new Ext.ux.netbox.date.DateOperator("DATE_NOT_EQUAL", "!=", format);
	op.addValidateFn(noEmptyAllowed);
	this.addOperator(op);
	op = new Ext.ux.netbox.date.DateOperator("DATE_ISNULL", "Empty", format);
	op.addValidateFn(noEmptyAllowed);
	this.addOperator(op);
	op = new Ext.ux.netbox.date.DateOperator("DATE_IS_NOT_NULL", "Not Empty", format);
	op.addValidateFn(noEmptyAllowed);
	this.addOperator(op);
...
Ext.extend(Ext.ux.netbox.core.RangeItem,Ext.menu.Adapter,{
...
Ext.ux.netbox.core.LocalStoreFilterResolver = function(filterModel, mapping) {
  ...
  if(mapping==undefined){
    this.mapping={
      ...
      DATE_NOT_EQUAL: { fn: this.filterDateNotEqual, scope: this }
    };
  }
...
Ext.ux.netbox.core.LocalStoreFilterResolver.prototype = {
  ...
  filterDateNotEqual: function (record, value, column) {
  	var date;
  	  	try {
	  		date = this.calcolateValueDate(value);
  	  	} catch (e) {
...
Ext.ux.netbox.core.QuickFilterModelView=function(config){
  ...
  this.dateOperDefault = ['DATE_EQUAL', 'DATE_GREATER', 'DATE_GREATER_OR_EQUAL', 'DATE_LESS', 'DATE_LESS_OR_EQUAL', 'DATE_NOT_EQUAL', 'DATE_ISNULL', 'DATE_IS_NOT_NULL'];
}
...
Ext.extend(Ext.ux.netbox.PreferenceManagerView, Ext.menu.Menu,
{
  ...
  imageRenderer : function(value, metadata, record, rowIndex, colIndex, store){
    if(value==true)
    	return ('<img class="x-menu-item-icon x-icon-checked" style="position: static; cursor: pointer" src="' + Ext.BLANK_IMAGE_URL + '"/>');
  },

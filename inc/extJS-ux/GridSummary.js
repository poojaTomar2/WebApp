    ...
    for (var j = 0, jlen = rs.length; j < jlen; j++) {
      ...
      for (var i = 0, len = cs.length; i < len; i++) {
        ...
        if (cf && cf.summaryType) {
        	...
        	if (this.grid.isSummary && summaryType == 'fixed'){
        		var jsonData = r.store.reader.jsonData;
        		data[c.name] = jsonData.Totals[c.name];
        	}
        	else {
        		data[c.name] = Ext.ux.grid.GridSummary.Calculations[summaryType](data[c.name] || 0, r, c.name, data);
        	}
        }
      }
    }
...
Ext.ux.grid.GridSummary.Calculations = {
  ...
  'groupSummaryTotals': function (v, record, field) {
	return v + Ext.num(record.data[field], 0);
  }

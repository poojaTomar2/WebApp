Cooler.AlternativeInstallReport = Ext.extend(Object, {
	regionSelected: null,
	...
	onExportButtonClick: function (button) {
		...
		var url = 'Controllers/AlternativeInstallReport.ashx?v=' + new Date();
		var params =
			{
				action: 'export',
				format: format,
				report: 'Alternative Install Report',
	...
	onShowButtonClick: function (button) {
		...
		if (this.regionCombo.selectedIndex >= 0) {
			...
			this.mask.show();
			Ext.Ajax.request({
				url: 'Controllers/AlternativeInstallReport.ashx',
				params: params,
	...
	createFilterPanel: function () {
		if (!this.panel) {
			...
			var typeOfReport = [[1, 'New Coolers'], [2, 'Retrofit Coolers']];
			var typeOfReportCombo = DA.combo.create({ fieldLabel: 'Type of report ', value: 2, name: 'ReportTypeId', hiddenName: 'ReportTypeId', store: typeOfReport, width: 180 });
			...
			this.panel = new Ext.form.FormPanel({
				title: 'Alternative Install Report',
				layout: 'border',
				closeAction: 'hide',
				closable: true,
				autoScroll: true,
...
Cooler.AlternativeInstallReport = new Cooler.AlternativeInstallReport();
Cooler.PowerSavingCommandReportByDevice = Ext.extend(Object, {
	...
	onExportButtonClick: function (button) {
		...
		var url = 'Controllers/PowerSavingCommandReportByDevice.ashx?v=' + new Date();
		var params = { action: 'export', format: format, report: 'Power Saving Command Report By Device', fromDate: reportCriteria.FromDate, toDate: reportCriteria.ToDate, clientId: reportCriteria.ClientId, countryId: reportCriteria.CountryId };
	...
	onShowButtonClick: function (button) {
		...
		Ext.Ajax.request({
			url: 'Controllers/PowerSavingCommandReportByDevice.ashx',
			params: params,
	...
	createFilterPanel: function () {
		if (!this.panel) {
			...
			var limitlocation = DA.Security.info.Tags.LimitLocation;
			var userCountryId = DA.Security.info.Tags.CountryId != 0;
			...
			var itemTpl = new Ext.XTemplate(
					'<div style = "padding: 15px">',
						'<tpl for = ".">',
							...
							'<br><br><br>',
							'<table class="report-table">',
								'<tr class="report-table-header-row"> <th>Status </th> <th> SmartTag </th> <th> SmartVision </th>  </tr>',
					...
					'</div>',
					{
						installDetail: function (countryDeviceDetail) {
							...
							countryDeviceDetail.forEach(function (data) {
								if (count == tempCount) {
									tempCount = 0;
									result += '<tr class="data-row lastRow" > <td>' + data.Status + '</td>  <td class="report-table-number-field">' + data.SmartTag + '</td> <td class="report-table-number-field">' + data.SmartVision + '</td>  </tr>';

								}
								else {
									result += '<tr class="data-row"> <td>' + data.Status + '</td> <td class="report-table-number-field">' + data.SmartTag + '</td> <td class="report-table-number-field">' + data.SmartVision + '</td>  </tr>';
									tempCount++;
								}
			...
			this.panel = new Ext.form.FormPanel({
				title: 'Power Saving Command Report By Device',
				layout: 'border',
				closeAction: 'hide',
				closable: true,
				autoScroll: true,
		...
		if (limitlocation != 0 && userCountryId) {
...
Cooler.PowerSavingCommandReportByDevice = new Cooler.PowerSavingCommandReportByDevice();
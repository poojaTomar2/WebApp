Cooler.CoolerImageIRVsClient = Ext.extend(Cooler.Form, {
	formTitle: 'IR VS Client QC: {0}',
	listTitle: 'IR VS Client QC',
	keyColumn: 'AssetPurityImageId',
	uniqueId: 'AssetPurityForClientIR',
	controller: 'AssetPurityImage',
	disableAdd: true,
	disableDelete: true,
	editable: true,
	securityModule: 'QualityCheckForIRVsClient',
	comboTypes: [
		'Product',
		'AssetPurityStatus'
	],
	constructor: function (config) {
		Cooler.CoolerImageIRVsClient.superclass.constructor.call(this, config || {});
	...
	hybridConfig: function () {
		...
		var items = [
			...
			{ dataIndex: 'RecogenitionTime', type: 'string', header: 'Recognition Time(Seconds)', width: 100 },
	...
	onComboLoad: function (me, countryId, stateId, clientId) {
		...
		var clientManualPurityStatus = record.get('ClientPurityStatus').indexOf(',') > 0 ? record.get('ClientPurityStatus').split(',') : [];
		if (Ext.isEmpty(record.get('ClientPurityStatus'))) {
			clientManualPurityStatus = record.get('ManualPurityStatus').indexOf(',') > 0 ? record.get('ManualPurityStatus').split(',') : [];
			if (clientManualPurityStatus.length != 0) {
				loadedFromtext = 'Self';
			}
		} else {
			loadedFromtext = record.dirty ? 'IR' : 'Client';
		}
		for (var row = 0; row < shelves; row++) {
			for (var column = 0; column < columns; column++) {
				var productCombo = DA.combo.create({
					...
					cls: (clientManualPurityStatus.length != 0 && clientManualPurityStatus.length >= pos && Number(purityStatus[pos]) != Number(clientManualPurityStatus[pos])) ? 'IRvsManualComboNotMatched' : '',
					listWidth: 250,
					disabled: true,
		...
		for (var row = 0; row < shelves; row++) {
			for (var column = 0; column < columns; column++) {
				var comboValue = Number(clientManualPurityStatus[manualPurityPos] || Cooler.Enums.Product.Unknown);
			...
			comboContainer.add({
				...
				tbar: ['Ground Truth Loaded from : ' + loadedFromtext, syncButton],
	...
	reloadCombos: function (me, record) {
		...
		Cooler.CoolerImageIRVsClientManual.onComboLoad(me, record.get('CountryId'), record.get('StateId'), record.get('ClientId'));
	},
	...
	onPlanogramDataLoaded: function (form, data) {
		...
		Cooler.CoolerImageIRVsClientManual.onComboLoad(this, this.currentRecord.get('CountryId'), this.currentRecord.get('StateId'), this.currentRecord.get('ClientId'));
	},
			...
			listeners: {
				render: function (image) {
					image.el.on('load', function (evt) {
						var me = Cooler.CoolerImageIRVsClientManual;
	...
	onRowImageClick: function (grid, rowIndex, e) {
		...
		Cooler.CoolerImageIRVsClientManual.grid.getSelectionModel().selectRow(rowIndex);

	},

	checkUnProcessedImage: function () {
		var records = Cooler.CoolerImageIRVsClientManual.grid.getStore().data, toReturn = false;
	...
	//	var records = Cooler.CoolerImageIRVsClientManual.grid.getStore().data, imageId = 0;
	//	if (btn === 'yes') {
	//		clearTimeout(this.timeout);
	//		this.assignNewImage();
	//		for (var i = 0; i < records.items.length; i++) {
	//			if (!records.get(i).get('VerifiedOn') && records.get(i).get('VerifiedOn') === "") {
	//				imageId = records.get(i).get('AssetPurityImageId');
	//				break;
	//			}
	//		}
	//		Ext.Ajax.request({
		...
		Ext.MessageBox.show({
			...
			fn: function (btn) {
				if (btn == 'yes') {
					...
					this.saveMask.show();
					Cooler.CoolerImageIRVsClientManual.onSave(btn, record, this);
				}
			},
			scope: this
		});
	...
	onSave: function (btn, record, me) {
		...
		var params = {
			...
			ClientPurityStatus: manualPurityStatus.join(','),
	...
	configureListTab: function (config) {
		...
		Ext.EventManager.onWindowResize(function () {
			image_width = 105, image_height = 15;
			Ext.getCmp('my-data-view').setHeight(image_width - image_height);
			Ext.getCmp('my-data-view').setWidth(Cooler.CoolerImageIRVsClientManual.productsImage.getStore().data.items.length * image_width);
		})
	},
	syncPurityCombo: function () {
		...
		if (!Ext.isEmpty(purityStatus)) {
			// Loading ClientPurityStatus from PurityStatus
			record.set('ClientPurityStatus', record.get('PurityStatus'));
			this.currentRecord = record;
		}
		Cooler.CoolerImageIRVsClientManual.onComboLoad(this, record.get('CountryId'), record.get('StateId'), record.get('ClientId'));
	},
...
Cooler.CoolerImageIRVsClientManual = new Cooler.CoolerImageIRVsClient({
	uniqueId: 'AssetPurityForIRVsClient', gridPlugins: [new DA.form.plugins.Inline({
		modifiedRowOptions: {
			fields: 'modified'
		}
	})]

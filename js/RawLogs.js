Cooler.RawLogs = new Cooler.Form({
	title: 'Raw Log',
	controller: 'RawLogs',
	disableAdd: true,
	allowExport: false,
	securityModule: 'RawLogs',
	gridConfig: {
	...
	hybridConfig: function () {
		return [
			...
			{ header: 'Serial#', dataIndex: 'SerialNumber', type: 'string', width: 50, sortable: false, hyperlinkAsDoubleClick: true, menuDisabled: true },
			{ header: 'Session', dataIndex: 'Session', type: 'string', width: 260, sortable: false, filterable: true },
			{ header: 'Category', dataIndex: 'Category', type: 'string', width: 450, sortable: false, filterable: true },
			{ header: 'Asset#', dataIndex: 'AssetSerialNumber', type: 'string', sortable: false, hyperlinkAsDoubleClick: true, menuDisabled: true },
			{ header: 'Location#', dataIndex: 'LocationCode', type: 'string', sortable: false, menuDisabled: true },
			{ header: 'Location', dataIndex: 'LocationName', type: 'string', width: 200, sortable: false, menuDisabled: true },
			{ header: 'City', dataIndex: 'City', type: 'string', sortable: false, menuDisabled: true },
			{ header: 'State', dataIndex: 'State', type: 'string', sortable: false, menuDisabled: true },
			{ header: 'Country', dataIndex: 'Country', type: 'string', sortable: false, menuDisabled: true },
			{ header: 'Client', dataIndex: 'Client', type: 'string', sortable: false, menuDisabled: true },
		...
		gridConfig.custom.tbar = [
			...
			'Category',
			{
				...
				store: [
					['', 'All'],
					['"Serial#"', 'Registration'],
					['"Logging raw data"', 'Communication'],
					['Command AND Data', 'Event Data'],
					['Command AND Image', 'Image'],
					['Command AND CellDump', 'Cell Dump'],
					['Command AND SmartDevicePing', 'Ping'],
					['Command AND DeviceAssetData', 'Device Installtion'],
					['Command AND CommandResponse', 'Command Response'],
					['Command AND WhitelistDevices', 'Whitelist Devices'],
					['Command AND ListRemoteCommands', 'Remote Commands'],
					['"Invalid date/time"', 'Invalid Date/Time'],
					['"Invalid Serial"', 'Invalid Serial'],
					['"Power Event Missed"', 'Missing Power Event'],
					['Error', 'Error']
				],
				plugins: [
		...
		for (var i = 0, len = fields.length; i < len; i++) {
			...
			if (field.name === fieldToSet) {
				if (valueToSet && typeof (valueToSet) == "string") {
					field.setValue(valueToSet.toUpperCase());
				} else {
					field.setValue('');
				}
				break;

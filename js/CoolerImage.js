Cooler.CoolerImageForm = Ext.extend(Cooler.Form, {
	formTitle: 'Cooler Image: {0}',
	listTitle: 'Cooler Images',
	keyColumn: 'AssetPurityId',
	controller: 'AssetPurity',
	disableAdd: true,
	disableDelete: true,
	editable: true,
	securityModule: 'CoolerImages',
	comboTypes: [
		'Product',
		'AssetPurityStatus'
	],
	constructor: function (config) {
		Cooler.CoolerImageForm.superclass.constructor.call(this, config || {});
	...
	hybridConfig: function () {
		...
		var items = [
			...
			{ dataIndex: 'SerialNumber', type: 'string', header: 'Serial Number', width: 120, hyperlinkAsDoubleClick: true },
			...
			{ dataIndex: 'CreatedOn', type: 'date', header: 'Image Received On(UTC)', width: 140, renderer: Cooler.renderer.DateTimeWithLocalTimeZone, convert: Ext.ux.DateLocalizer },
			...
			{ dataIndex: 'PurityDateTime', type: 'date', header: 'Image Capture Time', width: 140, renderer: ExtHelper.renderer.DateTime },
			...
			{ dataIndex: 'Provisioned', type: 'string', header: 'Provisioned?' }
		];
		...
		items.push({ dataIndex: 'PurityStatus', type: 'string' },
			...
			{ dataIndex: 'ProcessTypeName', type: 'string', header: 'Process Type' },
			//{ dataIndex: 'Location', type: 'string', header: 'Outlet', width: 160 },
			{ dataIndex: 'ClientCode', type: 'string', header: 'CoolerIot Client' },
			...
			{ dataIndex: 'ImageCount', type: 'int', header: 'Image Count' },
	...
	onComboLoad: function (me, countryId, stateId, clientId) {
		...
		if (productData) {
			for (var i = 0, len = productData.length; i < len; i++) {
				...
				else if ((Ext.isEmpty(clientId) || record.ClientId === 0 || record.ClientId === currentClientId) && (Ext.isEmpty(countryId) || countryId === 0 || record.CountryId === 0 || record.CountryId === countryId) && (Ext.isEmpty(stateId) || stateId === 0 || record.StateId === 0 || record.StateId === stateId)) {
		...
		if (Ext.isEmpty(purityStatus)) {
			purityStatus = this.previousPurityStatus ? this.previousPurityStatus : [];
		} else {
			this.previousPurityStatus = purityStatus;
		}
	...
	reloadCombos: function (me, record) {
		...
		Cooler.CoolerImage.onComboLoad(me, record.get('CountryId'), record.get('StateId'), record.get('ClientId'));
	},
	onRowClick: function (grid) {
		...
		} else if (origImageName && origImageName.split('_').length > 1) {
		...
		if (planogramId) {
			...
			if (creatingForm) {
				...
				planogram.on('dataLoaded', this.onPlanogramDataLoaded, this);
			}
	...
	onPlanogramDataLoaded: function (form, data) {
		...
		Cooler.CoolerImage.onComboLoad(this, this.currentRecord.get('CountryId'), this.currentRecord.get('StateId'), this.currentRecord.get('ClientId'));
	},
	...
	onRowImageClick: function (grid, rowIndex, e) {
		...
		Cooler.CoolerImage.grid.getSelectionModel().selectRow(rowIndex);

	},

	checkUnProcessedImage: function () {
		var records = Cooler.CoolerImage.grid.getStore().data, toReturn = false;
	...
	unAssignImage: function (btn) {
		var records = Cooler.CoolerImage.grid.getStore().data, imageId = 0;
		if (btn === 'yes') {
	...
	showStatistics: function () {
		...
		// removed un assignment because it is not working properly on test site
		//if (selectedRecord && !DA.Security.HasPermission('Admin')) {
		//	var assignedImageTimeDiff = this.getAssignedDateDiffrence(selectedRecord.get('AssignedOn'));
		//	if (assignedImageTimeDiff > 1 && selectedRecord.get('VerifiedOn') === "") { // if Image assigned previously and then unassign it and assign new one
		//		this.unAssignImage('yes');
		//	}
		//}
		if (store.data.length <= 0) {
			this.assignNewImage(); // if No image assign then it will assign new Image to operator
	...
	onBeforeSave: function (btn) {
		...
		Cooler.CoolerImage.onSave(btn, record, this);
	},
	...
	configureListTab: function (config) {
		...
		var btnSyncPlanogram = new Ext.Toolbar.Button({ text: 'Sync Planogram', handler: this.syncPlanogram, scope: this, iconCls: 'btn-icon-planogram' });
		...
		var btnProportionateImage = new Ext.Toolbar.Button({ text: '1:1', handler: this.resetZoom, scope: this });
		...
		var btnArrangeImages = new Ext.Toolbar.Button({ text: 'Arrange', handler: this.arrangeImages, iconCls: 'btn-icon-Arrange', scope: this });
		...
		var imagePreview = new Ext.Panel({
			...
			overflowX: 'scroll',
			cls: 'tbarCls',
			tbar: [btnDownloadImage, btnSaveImage, btnSyncPlanogram, btnArrangeImages, btnZoomOut, btnZoomIn, btnProportionateImage, 'Light ON?', lightOn, 'Non beverage items?', nonBeverageItem, btnCoolerImageId, btnAsset, btnSerialNumber, btnAssetLocation, btnProcessedImages, btnStatistics],
			items: [
				new Ext.Panel({
					region: 'center',
					layout: 'fit',
		...
		this.productsGrid = this.getProductList();
		...
		Ext.apply(config, {
			...
			items: [westPanel, imagePreview, this.productsGrid, panelProductsImage]
		});
		Ext.EventManager.onWindowResize(function () {
			image_width = 105, image_height = 15;
			Ext.getCmp('my-data-view').setHeight(image_width - image_height);
			Ext.getCmp('my-data-view').setWidth(Cooler.CoolerImage.productsImage.getStore().data.items.length * image_width);
		})
	},
					...
					},
					{
						...
						items: [
							imageCombo,
							{
								xtype: 'button',
								text: 'Crop',
								handler: Cooler.CoolerImage.cropImage,
				...
				tbar: [
					{
						xtype: 'button',
						text: 'Ok',
						handler: Cooler.CoolerImage.updateImages,
						iconCls: 'save',
						scope: this
					},
					{
	...
	cropImage: function () {
		...
		if (count > 0) {
			...
			var cropWindow = new Ext.ux.CropWindow({
				...
				parent: this,
				listeners: {
					saveCrop: Cooler.CoolerImage.onSaveCrop,
	...
	onSaveCrop: function (copWin, coodinates) {
		...
		Ext.Ajax.request({
			url: EH.BuildUrl('AssetPurity'),
			params: params,
			success: Cooler.CoolerImage.onCropSucces,
			failure: function () {
	...
	updateImages: function () {
		if (this.grid.getSelectionModel()) {
			...
			Ext.Ajax.request({
				...
				failure: Cooler.CoolerImage.onArrangeImageFailure,
				scope: this
			});
			if (!this.mask) {
	...
	getProductList: function () {
		var store = this.comboStores.Product;
		var grid = new Ext.grid.GridPanel({
			store: store,
			columns: [
				{ header: "Product", width: 110, sortable: true, dataIndex: 'DisplayValue' },
				{ header: "Images", width: 110, sortable: true, dataIndex: 'CustomStringValue' }
			],
			width: 85,
			region: 'east',
			split: true,
			...
			viewConfig: {
				getRowClass: function (record) {
					return record.get('SystemValue') && parseInt(record.get('SystemValue')) == -1 ? 'black-bold-row' : '';
				}
			},
			listeners: {
				'rowclick': this.onFocusProductImage,
				'rowdblclick': this.onProductGridDblClick,
				scope: this
			}
		});
		return grid;
	...
	onFocusProductImage: function (grid) {
		var productGrid = this.productsGrid;
		...
		//var imageLocation = location.href.concat('products/');
	...
	downloadCoolerImage: function (btn) {
		...
		var record = this.grid.getSelectionModel().getSelected();
		var imageUrl = EH.BuildUrl('DownloadCoolerImage') + "?fileName=" + imageName[0] + "&imageCount=" + record.data.ImageCount + "&assetPurityId=" + assetPurityId;
...
Cooler.CoolerImage = new Cooler.CoolerImageForm({
	uniqueId: 'AssetPurity', gridPlugins: [new DA.form.plugins.Inline({
		modifiedRowOptions: {
			fields: 'modified'
		}
	})]
});
Cooler.CoolerImageForAsset = new Cooler.CoolerImageForm({ uniqueId: 'AssetPurityForAsset', baseParams: { processed: 1 } });
Cooler.CoolerImageForAlert = new Cooler.CoolerImageForm({ uniqueId: 'AssetPurityForAlert', baseParams: { processed: 1 } });
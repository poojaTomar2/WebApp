Cooler.DeviceReport = Ext.extend(Object, {
	...
	onExportButtonClick: function (button) {
		...
		var url = 'Controllers/DeviceReport.ashx?v=' + new Date();
		var params = { action: 'export', format: format, report: 'Install Device Report', fromDate: reportCriteria.FromDate, toDate: reportCriteria.ToDate, clientId: reportCriteria.ClientId, countryId: reportCriteria.CountryId };
	...
	onShowButtonClick: function (button) {
		...
		Ext.Ajax.request({
			url: 'Controllers/DeviceReport.ashx',
			params: params,
			...
			var itemTpl = new Ext.XTemplate( 
					'<div style = "padding: 15px">',
						'<tpl for = ".">',
							...
							'<br><br><br>',
							'<table class="report-table">',
								'<tr class="report-table-header-row"> <th>Outlet Type</th><th>Country</th> <th>Beacon</th> <th>Proximity</th> <th>Proximity + camera</th> <th>Always-On</th> <th>Always On + camera</th>  <th>Proximity built-in</th> <th>Always-On Built-In</th>  <th>Warehouse</th><th>Total Count</th></tr>',
					...
					'</div>',
				{
					installDetail: function (countryDeviceDetail) {
							...
							countryDeviceDetail.forEach(function (data) {
								if (count == tempCount) {
									tempCount = 0;
									result += '<tr class="data-row lastRow" > <td>' + data.OutletType + '</td> <td>' + data.Country + '</td> <td class="report-table-number-field">' + data.Beacon + '</td> <td class="report-table-number-field">' + data.Proximity + '</td> <td class="report-table-number-field">' + data.ProximityAndCamera + '</td> <td class="report-table-number-field">' + data.AlwaysOn + '</td> <td class="report-table-number-field">' + data.AlwaysOnAndCamera + '</td> <td class="report-table-number-field">' + data.ProximityBuiltIn + '</td> <td class="report-table-number-field">' + data.AlwaysOnBuiltIn + '</td>  <td class="report-table-number-field">' + data.Warehouse + '</td> <td class="report-table-number-field">' + data.TotalCount + '</td></tr>';
								}
								else {
									result += '<tr class="data-row"> <td>' + data.OutletType + '</td> <td>' + data.Country + '</td> <td class="report-table-number-field">' + data.Beacon + '</td> <td class="report-table-number-field">' + data.Proximity + '</td> <td class="report-table-number-field">' + data.ProximityAndCamera + '</td> <td class="report-table-number-field">' + data.AlwaysOn + '</td> <td class="report-table-number-field">' + data.AlwaysOnAndCamera + '</td> <td class="report-table-number-field">' + data.ProximityBuiltIn + '</td> <td class="report-table-number-field">' + data.AlwaysOnBuiltIn + '</td> <td class="report-table-number-field">' + data.Warehouse + '</td> <td class="report-table-number-field">' + data.TotalCount + '</td></tr>';
									tempCount++;
								}
...
Cooler.DeviceReport = new Cooler.DeviceReport();
Cooler.BatteryLevelReport = Ext.extend(Object, {
	...
	onExportButtonClick: function (button) {
		...
		var url = 'Controllers/BatteryLevelReport.ashx?v=' + new Date();
		var params = { action: 'export', format: format, report: 'Battery Level by DeviceType Report', fromDate: reportCriteria.FromDate, toDate: reportCriteria.ToDate, clientId: reportCriteria.ClientId, countryId: reportCriteria.CountryId };
	...
	onShowButtonClick: function (button) {
		...
		Ext.Ajax.request({
			url: 'Controllers/BatteryLevelReport.ashx',
			params: params,
	...
	createFilterPanel: function () {
		if (!this.panel) {
			...
			var itemTpl = new Ext.XTemplate(
				...
				'<br><br><br>',
				'<table class="report-table">',
				'<tr class="report-table-header-row"> <th>Battery Level</th> <th>SmartBeacon</th> <th>SmartTag</th> <th>SmartVision</th> </tr>',
				...
				'</div>',
				{
					installDetail: function (countryDeviceDetail) {
						...
						countryDeviceDetail.forEach(function (data) {
							if (count == tempCount) {
								tempCount = 0;
								result += '<tr class="data-row lastRow" > <td>' + data.BatteryLevel + '</td>  <td class="report-table-number-field">' + data.SmartBeacon + '</td> <td class="report-table-number-field">' + data.SmartTag + '</td> <td class="report-table-number-field">' + data.SmartVision + '</td></tr>';
							}
							else {
								result += '<tr class="data-row"> <td>' + data.BatteryLevel + '</td> <td class="report-table-number-field">' + data.SmartBeacon + '</td> <td class="report-table-number-field">' + data.SmartTag + '</td> <td class="report-table-number-field">' + data.SmartVision + '</td> </tr>';
								tempCount++;
							}
			...
			this.panel = new Ext.form.FormPanel({
				title: 'Battery Level by DeviceType Report',
				layout: 'border',
				closeAction: 'hide',
				closable: true,
				autoScroll: true,
...
Cooler.BatteryLevelReport = new Cooler.BatteryLevelReport();
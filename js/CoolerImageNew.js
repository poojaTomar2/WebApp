Cooler.CoolerImageNew = new Cooler.Form({
	formTitle: 'Cooler Image: {0}',
	listTitle: 'Cooler Images New',
	...
	disableDelete: true,
	editable: true,
	securityModule: 'CoolerImagesDoubleScreen',
	comboTypes: [
		'Product',
		'AssetPurityStatus'
	],
	...
	hybridConfig: function () {
		...
		return [
			...
			{ dataIndex: 'ConsumerEmail', type: 'string', header: 'Username' },
			...
			{ dataIndex: 'CreatedOn', type: 'date', header: 'Image Received On', width: 140, convert: Ext.ux.DateLocalizer, renderer: Cooler.renderer.DateTimeWithLocalTimeZone },
			{ dataIndex: 'VerifiedOn', type: 'date', header: 'Recognition time', width: 140, renderer: ExtHelper.renderer.DateTime, convert: Ext.ux.DateLocalizer, renderer: Cooler.renderer.DateTimeWithLocalTimeZone },
			...
			{ dataIndex: "StatusId", type: "int", header: 'Status', width: 100, xtype: 'combocolumn', editor: assetPurityStatusCombo, renderer: ExtHelper.renderer.Combo(assetPurityStatusCombo) },
	...
	onRowClick: function (grid) {
		...
		var imageArray = origImageName ? record.get('ImageName').split(',') : [];
		if (imageArray && imageArray.length > 1) {
			...
			for (var i = 0; i < totalImagesInScene; i++) {
				...
				this.src = this.src ? this.src + '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '" vertical-align= "middle" height = "50%"/>' : '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '"vertical-align= "middle" height = "50%"/>';
			}
		} else if (origImageName && origImageName.split('_').length > 1) {
			this.src = '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + origImageName + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '"/>';
		} else if (count > 1) {
			for (var i = 1; i <= count; i++) {
				var imageName = origImageName.replace('.jpg', '_' + i + '.jpg');
				if (!this.src) {
					this.src = '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '"vertical-align= "middle" height = "50%"/>';
				}
				else {
					this.src = this.src + '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '" vertical-align= "middle" height = "50%"/>';
				}
			}
		}
		else {
			this.src = '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + origImageName + '&ImageId=' + record.get('AssetPurityId') + '&v=' + new Date() + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '"/>';
		}
	...
	configureListTab: function (config) {
		...
		var westPanel = new Ext.Panel({ region: 'west', width: '60%', split: true, hidden: false, items: westItems, layout: 'fit' })
	...
	onArrangeImageSuccess: function (response, options) {
		...
		if (count > 1) {
			for (var i = 1; i <= count; i++) {
				var imageName = origImageName.replace('.jpg', '_' + i + '.jpg');
				if (!this.src) {
					this.src = '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '&v=' + new Date() + '"vertical-align= "middle" height = "50%"/>';
				}
				else {
					this.src = this.src + '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + imageName + '&ImageId=' + record.get('AssetPurityId') + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '&v=' + new Date() + '" vertical-align= "middle" height = "50%"/>';
				}
			}
		}
		else {
			this.src = '<img src = "Controllers/CoolerImagePreview.ashx?AssetImageName=' + origImageName + '&ImageId=' + record.get('AssetPurityId') + '&PurityDateTime=' + record.get('PurityDateTimeUtc').format(Cooler.Enums.DateFormat.PurityDate) + '&v=' + new Date() + '"/>';
		}
		this.openNewWindow(this.src);
		this.src = null;
		this.newWindow = null;

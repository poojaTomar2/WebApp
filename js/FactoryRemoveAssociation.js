Cooler.FactoryRemoveAssociation = Ext.extend(Cooler.Form, {
	controller: 'FactoryRemoveAssociation',
	keyColumn: 'SmartDeviceId',
	listTitle: 'Factory Remove Association',
	disableAdd: true,
	disableDelete: true,
	allowExport: false,
	securityModule: 'FactoryRemoveAssociation',
	constructor: function (config) {
		Cooler.FactoryRemoveAssociation.superclass.constructor.call(this, config || {});
	...
	onRemoveAssociationButtonClick: function (sender) {
		...
		Ext.Msg.confirm("Alert", "You want to Remove Device Association of  <b>" + smartDeviceTypeNames + "</b>&nbsp;&nbsp;?", function (btnText) {
			if (btnText === "yes") {
				...
				mask.show();
				Ext.Ajax.request({
					url: EH.BuildUrl('FactoryRemoveAssociation'),
					params: {
					...
					success: function (result, request) {
						...
						Cooler.FactoryRemoveAssociation.grid.getStore().load();
					},
...
Cooler.FactoryRemoveAssociation = new Cooler.FactoryRemoveAssociation({ uniqueId: 'FactoryRemoveAssociation' });
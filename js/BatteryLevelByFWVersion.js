Cooler.BatteryLevelByFWVersion = Ext.extend(Object, {
	...
	onExportButtonClick: function (button) {
		...
		var url = 'Controllers/BatteryLevelByFWVersion.ashx?v=' + new Date();
		var params = { action: 'export', format: format, report: 'Battery Level by FWVersion Report', fromDate: reportCriteria.FromDate, toDate: reportCriteria.ToDate, clientId: reportCriteria.ClientId, countryId: reportCriteria.CountryId, deviceTypeId: reportCriteria.SmartDeviceTypeId };
	...
	onShowButtonClick: function (button) {
		...
		this.mask.show();
		var params = { action: 'list', fromDate: reportCriteria.FromDate, toDate: reportCriteria.ToDate, clientId: reportCriteria.ClientId, countryId: reportCriteria.CountryId, deviceTypeId: reportCriteria.SmartDeviceTypeId };
		if (params.deviceTypeId == "") {
			Ext.Msg.alert('Error', 'Please Select Device Type');

		}

		Ext.Ajax.request({
			url: 'Controllers/BatteryLevelByFWVersion.ashx',
			params: params,
	...
	createFilterPanel: function () {
		if (!this.panel) {
			...
			var deviceTypeCombo = DA.combo.create({ fieldLabel: 'Device Type', width: 180, hiddenName: 'SmartDeviceTypeId', allowBlank: false, baseParams: { comboType: 'SmartDeviceType' }, listWidth: 220, controller: "Combo" });
			...
			var itemTpl = new Ext.XTemplate(
					'<div style = "padding: 15px">',
						'<tpl for = ".">',
							...
							'<br><br><br>',
							'<table class="report-table">',
								'<tr class="report-table-header-row"> <th>FW Version </th> <th>0-25</th> <th>25-50</th> <th>50-75</th> <th>75-100</th> </tr>',
					...
					'</div>',
					{
						installDetail: function (countryDeviceDetail) {
							...
							countryDeviceDetail.forEach(function (data) {
								if (count == tempCount) {
									tempCount = 0;
									result += '<tr class="data-row lastRow" > <td>' + data.FWVersion + '</td>  <td class="report-table-number-field">' + data.Smart0to25 + '</td> <td class="report-table-number-field">' + data.Smart25to50 + '</td> <td class="report-table-number-field">' + data.Smart50to75 + '</td> <td class="report-table-number-field">' + data.Smart75to100 + '</td></tr>';
								}
								else {
									result += '<tr class="data-row"> <td>' + data.FWVersion + '</td> <td class="report-table-number-field">' + data.Smart0to25 + '</td> <td class="report-table-number-field">' + data.Smart25to50 + '</td> <td class="report-table-number-field">' + data.Smart50to75 + '</td> <td class="report-table-number-field">' + data.Smart75to100 + '</td> </tr>';
									tempCount++;
								}
			...
			this.panel = new Ext.form.FormPanel({
				title: 'Battery Level By FW Version',
				layout: 'border',
				closeAction: 'hide',
				closable: true,
				autoScroll: true,
...
Cooler.BatteryLevelByFWVersion = new Cooler.BatteryLevelByFWVersion();